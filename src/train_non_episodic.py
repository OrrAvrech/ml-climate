...
from models.base import ViTNonEpisodic
...
def main(cfg: TrainConfig):
    ...
    model = ViTNonEpisodic(cfg.model.name, device=device, num_classes=len(train_dataset.classes))
    # freeze backbone
    ...
    if cfg.model.train:
        ...
        for epoch in range(1, cfg.training.epochs + 1):
            ...
            for i, batch in enumerate(train_loader):
                ...
                if i % 10 == 0:
                    print(
                        f"epoch {epoch} - batch {i}, train, loss={loss.item():.4f} acc={acc:.4f}"
                    )
